...
function initTagInputs() {
  const catKnown = (state.filters.categories || []).map(c => c.value);
  const tagKnown = (state.filters.tags       || []).map(t => t.value);
  createTagInput('categoryTagInput', catKnown, true);
...
function setView(view) {
  ...
  $$('.filter-list-item').forEach(el => el.classList.remove('active'));
...
function renderSidebarFilters() {
  ...
  if (cats) {
    if (!state.filters.categories.length) {
      ...
      cats.innerHTML = state.filters.categories.map(c => {
        const safe = JSON.stringify(c.value).replace(/'/g, '&#39;');
        return `
          <div class="filter-list-item" onclick="window.PL_filterByCategory(${safe})">
  ...
  if (tags) {
    if (!state.filters.tags.length) {
      ...
      tags.innerHTML = state.filters.tags.map(t => {
        const safe = JSON.stringify(t.value).replace(/'/g, '&#39;');
        return `
          <div class="filter-list-item" onclick="window.PL_filterByTag(${safe})">
  ...
  updateTagInputKnown('categoryTagInput', (state.filters.categories || []).map(c => c.value));
...
function renderVariableFields(vars, meta) {
  ...
  wrap.innerHTML = visible.map(v => {
    ...
    return `<div class="var-field">
      <div class="var-field-label">${escapeHtml(v)}<span class="var-field-type">${type}</span></div>
      ${input}
    </div>`;
  }).join('');
}
...
let _chainRunnerState = { ids: [], step: 0, outputs: [], roles: {} }; // roles: { promptId: roleObj }

async function startChainRunner(chainIds) {
  _chainRunnerState = { ids: chainIds, step: 0, outputs: [], roles: {} };
  // Fetch role for each prompt that has one, cache by prompt id
...
function renderChainRunner() {
  const wrap   = $('#chainRunnerWrap');
  const runBtn = $('#runChainBtn');
  if (!wrap) return;
  const { ids, step, outputs } = _chainRunnerState;
  if (!ids.length) { wrap.style.display = 'none'; return; }

  if (runBtn) runBtn.style.display = 'none';
  wrap.style.display = 'block';

  const totalPhases = ids.length;
  const currentId   = ids[step];
  const currentP    = state.prompts.find(x => x.id === currentId);
  const vars        = currentP ? detectVariables(currentP.content || '') : [];
  const hasPrevOut  = step > 0 && outputs[step - 1];

  const phasesHtml = ids.map((id, i) => {
    const p      = state.prompts.find(x => x.id === id);
    const title  = p ? p.title : `Prompt #${id}`;
    const status = i < step ? 'done' : i === step ? 'current' : '';
    const varBlock = (i === step && vars.length > 0) ? `
      <div style="margin-top: var(--sp-3);">
        ${vars.map(v => `
          <div style="margin-bottom: var(--sp-2);">
            <div style="font-size: 11px; font-weight: 600; color: var(--ink-3); margin-bottom: 3px;">${escapeHtml(v)}</div>
            <input type="text" class="form-input chain-var-input" data-var="${escapeAttr(v)}" placeholder="Fill in…" style="padding: 6px 10px;" />
          </div>`).join('')}
      </div>` : '';
    const stepRole   = _chainRunnerState.roles[id];
    const roleBadge  = stepRole
      ? `<span style="display:inline-flex;align-items:center;gap:3px;font-size:10px;padding:2px 7px;border-radius:20px;background:var(--accent-soft);color:var(--accent);margin-top:3px;font-weight:600;">
           ${escapeHtml(stepRole.icon || '🎯')} ${escapeHtml(stepRole.name)}
         </span>`
      : '';
    return `
      <div class="chain-runner-step ${status}">
        <div style="display:flex; align-items:flex-start; gap: var(--sp-3);">
          <div class="chain-step-num-badge ${status}">${i < step ? '✓' : i + 1}</div>
          <div style="flex:1; min-width:0;">
            <div style="font-size: var(--fs-sm); font-weight: 600; color: var(--ink); margin-bottom: 2px;">${escapeHtml(title)}</div>
            <div style="font-size: 11px; color: var(--ink-3); margin-bottom: var(--sp-1);">Phase ${i + 1}${i < step && outputs[i] ? ' · response captured' : ''}</div>
            ${roleBadge}
            ${varBlock}
          </div>
        </div>
  ...
  const contextBar = hasPrevOut ? `
    <div class="chain-phase-context">
      <span class="material-symbols-outlined" style="font-size:14px;">link</span>
      <span>Phase ${step} response will be prepended as context</span>
    </div>` : '';

  const nextBtn = step < totalPhases - 1
    ? `<button class="btn btn-primary" onclick="window.PL_showOutputCapture()" style="justify-content:center;">Next phase <span class="material-symbols-outlined">arrow_forward</span></button>`
    : `<button class="btn" style="background:var(--success);color:white;border-color:var(--success);justify-content:center;" onclick="window.PL_finishChain()"><span class="material-symbols-outlined">check_circle</span> Finish chain</button>`;

  const copyLabel = `Copy Phase ${step + 1}${vars.length ? ' (fill variables first)' : ''}`;

  wrap.innerHTML = `
    <div class="chain-runner">
      <div class="chain-runner-header">
        <span>Phase ${step + 1} of ${totalPhases}</span>
      ...
      ${phasesHtml}
      ${contextBar}
      <div class="chain-output-capture" id="chainOutputCapture" style="display:none;">
        <div style="font-size: 11px; font-weight: 600; color: var(--ink-3); margin-bottom: var(--sp-2);">PASTE AI RESPONSE — Phase ${step + 1}</div>
        <textarea id="chainPhaseOutput" class="form-input" rows="5"
          placeholder="Paste the AI's response here — it becomes context for Phase ${step + 2}…"
          style="resize:vertical; font-size:var(--fs-sm); width:100%;"></textarea>
        <div style="display:flex; gap:var(--sp-2); margin-top:var(--sp-2);">
          <button class="btn btn-primary" style="flex:1; justify-content:center;" onclick="window.PL_captureAndNext()">
            Save &amp; go to Phase ${step + 2} <span class="material-symbols-outlined">arrow_forward</span>
          </button>
          <button class="btn btn-ghost" onclick="window.PL_skipCapture()" style="padding:6px 14px;">Skip</button>
        </div>
      </div>
      <div class="chain-runner-actions" id="chainRunnerActions">
        <button class="btn btn-accent" onclick="window.PL_copyChainPhase()" style="flex:1; justify-content:center;">
          <span class="material-symbols-outlined">content_copy</span>
          ${copyLabel}
        </button>
        ${nextBtn}
...
/* Copy current phase prompt — prepends previous AI response as context if captured */
window.PL_copyChainPhase = async function() {
  const { ids, step, outputs } = _chainRunnerState;
  const id = ids[step];
  const p  = state.prompts.find(x => x.id === id);
  if (!p) return;
  const varMap = {};
  $$('#chainRunnerWrap .chain-var-input').forEach(inp => {
    varMap[inp.dataset.var] = inp.value.trim() || `[${inp.dataset.var}]`;
  });
  const vars = detectVariables(p.content || '');
  let text   = vars.length ? replaceVariables(p.content, varMap) : (p.content || '');
  // Prepend role persona as system context if this prompt has a role assigned
  const role = _chainRunnerState.roles[id];
  if (role && role.persona) {
    const roleHeader = `--- Role: ${role.icon || ''} ${role.name} ---\n${role.persona}${role.tone ? '\nTone: ' + role.tone : ''}${role.expertise ? '\nExpertise: ' + role.expertise : ''}`;
    text = `${roleHeader}\n\n--- Prompt ---\n${text}`;
  }
  // Prepend previous phase output as context automatically
  if (step > 0 && outputs[step - 1]) {
    text = `--- Context from Phase ${step} ---\n${outputs[step - 1]}\n\n--- Phase ${step + 1} Prompt ---\n${text}`;
  }
  const ok = await copyToClipboard(text);
  if (ok) {
    api(`/prompts/${id}/use`, { method: 'POST' }).catch(() => {});
    toast(`Phase ${step + 1} copied — paste into your AI tool`, 'success');
  }
};

/* Show the AI response capture panel before advancing to next phase */
window.PL_showOutputCapture = function() {
  const { ids, step } = _chainRunnerState;
  if (step >= ids.length - 1) return;
  const capture = $('#chainOutputCapture');
  const actions  = $('#chainRunnerActions');
  if (capture) capture.style.display = 'block';
  if (actions)  actions.style.display  = 'none';
};

/* Save captured AI response and advance to next phase */
window.PL_captureAndNext = function() {
  const { ids, step } = _chainRunnerState;
  const outputText = $('#chainPhaseOutput')?.value?.trim() || '';
  _chainRunnerState.outputs[step] = outputText;
  if (step < ids.length - 1) {
    _chainRunnerState.step = step + 1;
    renderChainRunner();
    toast(`Phase ${_chainRunnerState.step + 1} ready`, 'info');
  }
};

/* Skip capture and advance without saving a response */
window.PL_skipCapture = function() {
  const { ids, step } = _chainRunnerState;
  _chainRunnerState.outputs[step] = '';
  if (step < ids.length - 1) {
    _chainRunnerState.step = step + 1;
    renderChainRunner();
    toast(`Phase ${_chainRunnerState.step + 1} ready`, 'info');
...
window.PL_nextChainStep = window.PL_showOutputCapture;
window.PL_copyChainStep = window.PL_copyChainPhase;

/* Completion summary with export option */
window.PL_finishChain = function() {
  const { ids, outputs } = _chainRunnerState;
  const wrap = $('#chainRunnerWrap');
  if (!wrap) return;
  const phaseSummary = ids.map((id, i) => {
    const p     = state.prompts.find(x => x.id === id);
    const title = p ? p.title : `Prompt #${id}`;
    const out   = outputs[i] || '';
    const preview = out ? `<div class="chain-output-preview">${escapeHtml(out.slice(0, 200))}${out.length > 200 ? '…' : ''}</div>` : '';
    return `
      <div class="chain-phase-summary">
    ...
    <div class="chain-runner">
      <div class="chain-runner-header" style="background:var(--success-soft);">
        <span style="color:var(--success);">
          ...
          Chain complete — ${ids.length} phase${ids.length !== 1 ? 's' : ''}
        </span>
...
/* Copy entire chain — all phases + captured responses — as a single block */
window.PL_exportFullChain = async function() {
  ...
  const parts = ids.map((id, i) => {
    ...
    let section   = `=== PHASE ${i + 1}: ${title.toUpperCase()} ===\n\n${content}`;
    if (outputs[i]) section += `\n\n--- AI Response (Phase ${i + 1}) ---\n${outputs[i]}`;
    return section;
  });
  const ok = await copyToClipboard(parts.join(divider));
  if (ok) toast('Full chain copied — paste into any AI tool or document', 'success');
};

window.PL_stopChain = function() {
  _chainRunnerState = { ids: [], step: 0, outputs: [] };
  const wrap   = $('#chainRunnerWrap');
  const runBtn = $('#runChainBtn');
...
async function handleCopyWithVariables() {
  ...
  try {
    ...
    const text = vars.length > 0 ? replaceVariables(p.content, map) : p.content;
    const ok = await copyToClipboard(text);
    if (ok) {
...
function openNewPromptModal() {
  ...
  resetTagInput('categoryTagInput');
...
async function editPrompt(id) {
  try {
    ...
    setTagInputValues('categoryTagInput', p.categories || []);
...
function switchEditorTab(name) {
  $$('.editor-tab').forEach(t => t.classList.toggle('active', t.dataset.pane === name));
  $$('.editor-pane').forEach(p => p.classList.toggle('active', p.id === `pane-${name}`));
}
...
async function handlePromptSubmit(e) {
  ...
  const data = {
    ...
    categories:   getTagInputValues('categoryTagInput').join(','),
...
function updateChainSelect(currentId) {
  ...
  sel.innerHTML = '<option value="">Select a prompt to add...</option>' +
    state.prompts.filter(p => p.id !== currentId)
...
function renderChatTurns(turns) {
  ...
  if (!turns.length) {
    wrap.innerHTML = '<p style="font-size: var(--fs-sm); color: var(--ink-3); margin-bottom: var(--sp-3);">No turns yet. Click <em>Add turn</em> below.</p>';
    return;
  }
...
function _switchImportFmt(fmt) {
  ...
  const panels = { json: '#importPanelJson', markdown: '#importPanelMarkdown', file: '#importPanelFile' };
...
async function exportJson() {
  try {
    ...
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    triggerDownload(blob, `prompts-${new Date().toISOString().slice(0, 10)}.json`);
    toast(`Exported ${data.length} prompts`, 'success');
    closeExportModal();
  } catch (err) {
...
async function exportFormat(path, mime, ext) {
  ...
  try {
    ...
    triggerDownload(blob, `prompts-${new Date().toISOString().slice(0, 10)}.${ext}`);
...
function showPremiumModal() {
  ...
  $('#licenceKeyInput').value = state.licenceKey || '';
...
window.PL_filterByCategory = function(value) {
  // Switch to library view without clearing the pill we're about to set
  ...
  $$('.filter-list-item').forEach(el => {
    el.classList.toggle('active', el.querySelector('span:nth-child(2)')?.textContent === value);
  });
};
...
function init() {
  ...
  $('#addChainBtn')?.addEventListener('click', addChainStep);
  $('#addChatTurnBtn')?.addEventListener('click', addChatTurn);
...
async function loadRoles(preserveActive) {
  try {
    const res  = await fetch('/api/roles');
    const data = await res.json();
    _rolesState.roles = data.roles || data || [];
    renderRolesList();
    updateRolesCount();
...
function renderRolesList() {
  ...
  list.innerHTML = roles.map(r => `
    <div class="role-list-item ${r.id === _rolesState.activeId ? 'active' : ''}"
      ...
      <span class="role-color-dot" style="background:${escapeAttr(r.colour || '#6366f1')}"></span>
...
function buildRolePrompt(role, format) {
  ...
  const example   = (role.example_phrase || '').trim();
  ...
  if (format === 'structured') {
    const parts = [];
    if (name)      parts.push(`## Role\n${icon} ${name}`);
    if (persona)   parts.push(`## Persona\n${persona}`);
    if (tone)      parts.push(`## Tone\n${tone}`);
    if (expertise) parts.push(`## Expertise\n${expertise}`);
    if (example)   parts.push(`## Example phrase\n"${example}"`);
  ...
  if (format === 'xml') {
    ...
    if (example)   parts.push(`  <example_phrase>${example}</example_phrase>`);
    if (includedKb.length) {
  ...
  if (format === 'prose') {
    ...
    if (example)   bits.push(`Example of how you speak: "${example}"`);
    if (includedKb.length) {
...
function getRoleFromForm() {
  ...
  return {
    ...
    example_phrase: $('#roleExampleInput')?.value  || '',
...
window.PL_newRole = function() {
  ...
  // Clear all fields
  ['#roleNameInput','#rolePersonaInput','#roleToneInput',
   '#roleExpertiseInput','#roleExampleInput'].forEach(sel => {
    const el = $(sel);
    if (el) el.value = '';
  });
  const iconBtn = $('#roleIconBtn');
...
function initRolesWorkspace() {
  ...
  const liveFields = ['#roleNameInput','#rolePersonaInput','#roleToneInput',
                      '#roleExpertiseInput','#roleExampleInput','#roleIconBtn'];
  ...
  // Icon picker — simple emoji prompt
  const iconBtn = $('#roleIconBtn');
  if (iconBtn) {
    iconBtn.addEventListener('click', () => {
      const val = prompt('Enter an emoji for this role:', iconBtn.textContent.trim());
      if (val && val.trim()) {
