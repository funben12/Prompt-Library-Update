    ...
    try {
      ...
      for (const p of ['openai', 'anthropic', 'gemini', 'openrouter']) {
        ...
        if (remote && remote !== local) {
          ...
          await api('/settings/ai-keys', { method: 'POST', body: { provider: p, key: local } });
        }
      }
    } catch (e) { console.warn('ai-key sync:', e); }
  })();
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', () => {
      ...
      if (key) {
        ...
        api('/settings/ai-keys', { method: 'POST', body: { provider, key } }).catch(() => {});
        const status = $('#configStatus');
        if (status) { status.textContent = 'Saved to your database.'; setTimeout(() => { status.textContent = ''; }, 2000); }
      } else {
...
{ cat: 'reasoning', icon: 'psychology_alt',      label: 'Perspective Shift',  text: 'Reframe the problem from a different perspective:\n- If you were [role or stakeholder], how would you approach this?\n- What assumptions would you challenge?\n- What alternative solutions might you consider?\n- How would your priorities or constraints differ?\n- What new insights emerge from this perspective?' },
  ...
  { cat: 'meta', icon: 'tune',                  label: 'Variable Extractor', text: 'Analyse this prompt and identify every element that should be a variable (things that change between uses). Return a list of variable names with descriptions.\n\nPrompt:\n[paste prompt here]\n\nVariables found:\n- [[variable_name]]: [what it represents]' },
  { cat: 'meta', icon: 'schema',                label: 'System Prompt Architect', text: 'Design a complete system prompt for an AI assistant with the following purpose:\n\nPurpose: [what the assistant does]\nPersonality: [tone and character]\nCapabilities: [what it can do]\nLimitations: [what it must not do]\nOutput style: [how it should respond]\n\nSystem prompt:' },
  { cat: 'meta', icon: 'splitscreen',           label: 'Persona Switch',     text: 'Respond to this as three different experts:\n\nExpert A — [role/lens]: [perspective]\nExpert B — [role/lens]: [perspective]\nExpert C — [role/lens]: [perspective]\n\nSynthesis: [combined recommendation]' },
  { cat: 'meta', icon: 'call_split',            label: 'Prompt Splitter',    text: 'Break the following complex task into focused sub-prompts, each of which can run independently.\n\nTask: [describe the complex task]\n\nSub-prompt 1 — [focus area]:\n[complete self-contained prompt]\n\nSub-prompt 2 — [focus area]:\n[complete self-contained prompt]\n\nExecution order: [sequential / parallel]\nHow to combine outputs: [instructions for merging]' },
  { cat: 'meta', icon: 'style',                 label: 'Tone Modifier',      text: 'Rewrite the following prompt to produce output in a different tone, without changing its instructions or intent.\n\nOriginal prompt:\n[paste prompt here]\n\nTarget tone: [e.g. warmer / more direct / more formal]\nReason for change: [what the original tone was getting wrong]\n\nRewritten prompt with new tone embedded:\n[output]' },
  { cat: 'meta', icon: 'grid_on',               label: 'Few-Shot Builder',   text: 'Construct a few-shot prompt for the following task using concrete examples.\n\nTask description: [what the AI should learn to do]\nFormat to demonstrate: [the exact output format]\n\nExample 1:\nInput: [realistic input]\nOutput: [ideal output]\n\nExample 2:\nInput: [realistic input]\nOutput: [ideal output]\n\nExample 3:\nInput: [realistic input]\nOutput: [ideal output]\n\nNow complete this:\nInput: [actual task input]\nOutput:' },
  { cat: 'meta', icon: 'account_tree',          label: 'Chain Designer',     text: 'Design a multi-step prompt chain for the following workflow.\n\nWorkflow goal: [what the chain should ultimately produce]\n\nStep 1 — [Name]:\nInput: [what this step receives]\nPrompt: [the prompt for this step]\nOutput: [what this step produces]\n\nStep 2 — [Name]:\nInput: [[step_1_output]]\nPrompt: [the prompt]\nOutput: [what this step produces]\n\nStep 3 — [Name]:\nInput: [[step_2_output]]\nPrompt: [the prompt]\nFinal output: [the end result]' },
  { cat: 'meta', icon: 'compress',              label: 'Prompt Compressor',  text: 'Compress the following prompt to be as short as possible while retaining every instruction and constraint.\n\nOriginal prompt ([N] words):\n[paste prompt here]\n\nCompressed prompt:\n[output]\n\nWords removed: [original count] → [compressed count]\nNothing lost: [confirm every instruction is preserved]' },
  ...
  { cat: 'meta', icon: 'compare',               label: 'AB Prompt Tester',   text: 'Generate two distinct versions of a prompt for A/B evaluation.\n\nTask: [describe the task]\nHypothesis: [what you think will perform better and why]\n\nVersion A — [approach name, e.g. Role-first]:\n[complete prompt]\n\nVersion B — [approach name, e.g. Task-first]:\n[complete prompt]\n\nEvaluation criteria:\n1. [criterion]\n2. [criterion]\n\nHow to test: [specific instructions for comparing]' },
  { cat: 'meta', icon: 'output',                label: 'Output Formatter',   text: 'Rewrite the following prompt to enforce a specific output format.\n\nOriginal prompt:\n[paste prompt here]\n\nRequired format: [JSON / Markdown table / numbered list / YAML / CSV]\n\nFormat specification:\n[show an exact example with placeholder values]\n\nRules:\n- Return ONLY the formatted output — no preamble\n- Every field must be present even if value is N/A\n\nRewritten prompt with format lock:\n[output]' },
  { cat: 'meta', icon: 'balance',               label: 'Bias Checker',       text: 'Analyse the following prompt for potential biases and suggest neutral alternatives.\n\nPrompt to check:\n[paste prompt here]\n\nPotential biases identified:\n1. [bias 1 — why it is biased]\n2. [bias 2 — why it is biased]\n3. [bias 3 — why it is biased]\n\nNeutralised prompt:\n[rewritten prompt with bias mitigated]\n\nExplanation of changes:\n- [change 1 and rationale]\n- [change 2 and rationale]' },
  { cat: 'meta', icon: 'flip_camera_android',   label: 'Perspective Shifter', text: 'Rewrite the following prompt from a different perspective or role.\n\nOriginal prompt:\n[paste prompt here]\n\nNew perspective: [e.g. customer, competitor, regulator, novice user]\n\nRewritten prompt:\n[output]\n\nHow the perspective changes the framing:\n- [point 1]\n- [point 2]' },
  ...
  function _pcwSaveDraft() {
    try {
      ...
      localStorage.setItem('pl_pcw_draft', JSON.stringify({ blocks: _canvasBlocks, title: t, view: _canvasState }));
    } catch (e) { /* storage full — draft skipped */ }
  }

  // Adopt the expanded dataset from components-data.js when present.
  // Inline arrays above remain as the offline fallback.
  var _PCW_CAT_COLORS = {
    core: 'var(--accent)',   reasoning: 'var(--c-purple)', control: 'var(--c-orange)',
    output: 'var(--c-blue)', writing: 'var(--c-pink)',     analysis: 'var(--c-green)',
    meta: 'var(--c-yellow)', guardrails: 'var(--c-red)',   agentic: '#06b6d4',
    dialogue: '#8b5cf6',     ux: '#a855f7',                creative: '#f43f5e',
    ideation: '#fb923c',     coding: '#10b981',            business: '#f59e0b',
    data: '#3b82f6',         personas: '#ec4899',          education: '#14b8a6',
    medical: '#ef4444',      legal: '#64748b',             research: '#0ea5e9',
    social: '#22c55e',       technical: '#6366f1',         misc: '#9ca3af'
  };
  if (window._pcwBLOCKS && window._pcwBLOCKS.length) {
    ...
    var _fwSeen = {};
    FRAMEWORKS.forEach(function(f) { _fwSeen[String(f.badge).toLowerCase()] = 1; });
    _extFw.forEach(function(f) {
      var k = String(f.badge).toLowerCase();
      if (!_fwSeen[k]) { _fwSeen[k] = 1; FRAMEWORKS.push(f); }
    });
    FRAMEWORKS.sort(function(a, b) { return String(a.badge).localeCompare(String(b.badge)); });
    ...
    if (window._pcwCATEGORIES && window._pcwCATEGORIES.length >= CATEGORIES.length) {
      CATEGORIES = window._pcwCATEGORIES.map(function(c) {
        return { id: c.id, label: c.label, icon: c.icon, desc: c.desc || '', color: _PCW_CAT_COLORS[c.id] || 'var(--accent)' };
      });
    }
  }
  ...
  function _pcwUpdateStats() {
    ...
    var blanks = 0;
    _canvasBlocks.forEach(function(b) { blanks += countBlanks(b.text); });
  ...
  function renderCatRail() {
    var rail = $('#pcwCatRail');
    if (!rail) return;
    var allCats = [{ id: 'all', label: 'All categories', icon: 'apps', color: 'var(--accent)' }].concat(CATEGORIES);
  ...
  function _pcwOrdered() {
    return _canvasBlocks.map(function(b, i) { return { b: b, i: i }; }).sort(function(p, q) {
      var ya = p.b.y || 0, yb = q.b.y || 0;
  ...
  function _pcwNextPos() {
    ...
    var vw = zone ? zone.clientWidth : 900, vh = zone ? zone.clientHeight : 600;
    ...
    return { x: cx + (n % 5) * 26, y: cy + (n % 5) * 26 };
  }
  ...
  function renderCanvas() {
    ...
    world.innerHTML = '';
    var order = {};
    _pcwOrdered().forEach(function(o, pos) { order[o.i] = pos + 1; });
    ...
    _canvasBlocks.forEach(function(b, idx) {
      ...
      hdr.addEventListener('mousedown', function(e) {
        ...
        var origX = _canvasBlocks[idx].x, origY = _canvasBlocks[idx].y;
        var moved = false;
        function onMove(ev) {
  ...
  function addBlock(label, text, cat, pos) {
    ...
    _canvasBlocks.push({ label: label, text: text, cat: cat || null, collapsed: false, x: p.x, y: p.y });
    renderCanvas();
  }

  function _pcwCatColor(cat) {
    var m = CATEGORIES.filter(function(c) { return c.id === cat; })[0];
  ...
  function assemblePrompt() {
    return _pcwOrdered().map(function(o) { return o.b.text.trim(); }).filter(Boolean).join('\n\n');
  }
  ...
  async function saveToLibrary() {
    ...
    try {
      ...
      await api('/prompts', {
        method: 'POST',
        body: { title: title, content: assembled, description: 'Built with Prompt Components', tags: tags, categories: '', folder_id: folderId }
      });
  ...
  function _galPush(view) { _galStack.push(view); _galRender(); }
  function _galBack() {
    if (_galStack.length > 1) { _galStack.pop(); _galRender(); }
    else _galClose();
  }
  ...
  function _galCheckout() {
    ...
    _galCart.forEach(function(it, i) {
      _canvasBlocks.push({
        label: it.label, text: it.text, cat: it.cat || null,
        collapsed: i < _galCart.length - 1,
        x: base.x, y: base.y + i * 70
      });
    });
  ...
  function _galItemFromBlock(b) {
    return { label: b.label, text: b.text, cat: b.cat, icon: b.icon };
  }
  function _galItemFromFw(f) {
    return { label: f.name, text: f.text || '', cat: null, icon: 'extension', badge: f.badge };
  }
  ...
  function _galRender() {
    ...
    if (v.mode === 'home') {
      ...
      var cm = CATEGORIES.filter(function(c) { return c.id === v.cat; })[0];
      if (title) title.textContent = cm ? cm.label : 'Category';
      if (icon) icon.textContent = cm ? cm.icon : 'category';
      var list = [];
      BLOCKS.forEach(function(b, gi) { if (b.cat === v.cat) list.push(_galBlockCard(b, gi)); });
  ...
  function _aiCatalogue() {
    return CATEGORIES.map(function(c) {
      var labels = BLOCKS.filter(function(b) { return b.cat === c.id; }).map(function(b) { return b.label; });
  ...
  function _aiFindBlock(label) {
    ...
    if (!l) return null;
    var exact = BLOCKS.filter(function(b) { return b.label.toLowerCase() === l; })[0];
  ...
  async function _aiAsk() {
    ...
    if (text.length < 10) { _aiStatus('Describe the prompt in a little more detail first.', true); return; }
    ...
    try {
      ...
      _aiPlan = { options: options, chosen: 0 };
      var wrap = $('#pcwAiOptions');
      if (wrap) {
  ...
  function _aiWait(ms) { return new Promise(function(r) { setTimeout(r, ms); }); }
  ...
  function _aiGhostGlide(fromEl, sx, sy, ms) {
    return new Promise(function(res) {
      ...
      setTimeout(function() { g.remove(); res(); }, ms + 30);
    });
  }
  ...
  async function _aiBuild(replace) {
    ...
    var items = opt.items;
    _aiClose();
    if (replace) { _canvasBlocks = []; renderCanvas(); }
    ...
    try {
      for (var i = 0; i < items.length; i++) {
        var b = items[i];
        var tx = 80, ty = 80 + (startCount + i) * 70;
        var wsOpen = ws && ws.classList.contains('open');
        if (_aiSkip || reduced || !wsOpen) {
          _galClose();
          _canvasBlocks.push({ label: b.label, text: b.text, cat: b.cat, collapsed: i < items.length - 1, x: tx, y: ty });
          continue;
        }
        ...
        if (first) {
          ...
          if (dept) {
            dept.scrollIntoView({ block: 'center' });
            await _aiCursorToEl(dept, 340 * speed);
            await _aiCursorPress(dept);
          }
          _galPush({ mode: 'cat', cat: b.cat });
        } else {
          _galOpen({ mode: 'cat', cat: b.cat });
        }
        ...
        if (galCard) {
          galCard.scrollIntoView({ block: 'center' });
          await _aiWait(60);
        ...
        _galClose();
        await _aiWait(80);
        var zr = zone ? zone.getBoundingClientRect() : { left: 100, top: 100 };
  ...
  window.openComponentsWorkspace = function() {
        ...
        if (b) {
          if (typeof window.openPreviewModal === 'function') {
            window.openPreviewModal({ icon: b.icon, title: b.label, text: b.text,
              ...
              onInsert: function(text) { addBlock(b.label, text, b.cat); wireDropZone(); } });
          } else { addBlock(b.label, b.text, b.cat); wireDropZone(); }
        }
      } else if (fw) {
    ...
    if (zoneEl) {
      zoneEl.addEventListener('mousedown', function(e) {
        ...
        var ox = _canvasState.panX, oy = _canvasState.panY;
    ...
    if ($('#pcwZoomInBtn'))  $('#pcwZoomInBtn').addEventListener('click', function() {
      _canvasState.zoom = Math.min(2, _canvasState.zoom * 1.2); _pcwApplyView(); _pcwSaveDraft();
    });
    if ($('#pcwZoomOutBtn')) $('#pcwZoomOutBtn').addEventListener('click', function() {
      _canvasState.zoom = Math.max(0.25, _canvasState.zoom / 1.2); _pcwApplyView(); _pcwSaveDraft();
    });
    if ($('#pcwFitBtn'))     $('#pcwFitBtn').addEventListener('click', _pcwFitView);

    // Component gallery wiring
    function _openGalleryHome() { _galOpen({ mode: 'home' }); }
    ...
    function _galShowCart() {
      var top = _galStack[_galStack.length - 1];
      if (!top || top.mode !== 'cart') _galPush({ mode: 'cart' });
    }
    ...
    if ($('#pcwGallery')) {
      $('#pcwGallery').addEventListener('keydown', function(e) {
        if (e.key === 'Escape') { e.stopPropagation(); _galBack(); }
      });
    }
    if ($('#pcwGalBody')) {
      $('#pcwGalBody').addEventListener('click', function(e) {
        ...
        if ((el = e.target.closest('[data-cart-block]'))) {
          ...
          _galPush(cid === '__fw' ? { mode: 'fw' } : { mode: 'cat', cat: cid });
        } else if ((el = e.target.closest('[data-gal-block]'))) {
          var gb = BLOCKS[parseInt(el.dataset.galBlock, 10)];
          if (gb) _galPush({ mode: 'preview', item: gb });
        } else if ((el = e.target.closest('[data-gal-fw]'))) {
          var gf = FRAMEWORKS[parseInt(el.dataset.galFw, 10)];
          if (gf) _galPush({ mode: 'preview', item: { label: gf.name, name: gf.name, icon: 'extension', text: gf.text || '', badgeLabel: gf.badge, cat: null, fw: true } });
          ...
          if (v && v.mode === 'preview' && v.item) {
            var it = v.item;
            _galCartAdd({ label: it.label || it.name, text: it.text, cat: it.cat || null, icon: it.icon, badge: it.badgeLabel || it.badge });
            _galBack();
          }
        }
      });
    ...
    if ($('#pcwGallerySearch')) {
      $('#pcwGallerySearch').addEventListener('input', function(e) {
        ...
        var top = _galStack[_galStack.length - 1];
        if (q.length >= 2) {
          if (top && top.mode === 'search') { top.q = q; _galRender(); }
          else _galPush({ mode: 'search', q: q });
    ...
    if ($('#pcwAiOverlay')) {
      ...
      $('#pcwAiOverlay').addEventListener('keydown', function(e) {
        if (e.key === 'Escape') { e.stopPropagation(); _aiClose(); }
      });
    }
    if ($('#pcwAiLevels')) {
    ...
    if ($('#pcwAiBackBtn'))    $('#pcwAiBackBtn').addEventListener('click', function() { _aiSetStep('ask'); });
    if ($('#pcwAiGoBtn'))      $('#pcwAiGoBtn').addEventListener('click', function() { _aiBuild(false); });
    if ($('#pcwAiReplaceBtn')) $('#pcwAiReplaceBtn').addEventListener('click', function() { _aiBuild(true); });
    if ($('#pcwAiSkip'))       $('#pcwAiSkip').addEventListener('click', function() { _aiSkip = true; });
    ...
    ws.addEventListener('click', function(e) {
      ...
      (recipes[qs.dataset.qs] || []).forEach(function(lbl) {
        var b = BLOCKS.filter(function(x) { return x.label === lbl; })[0];
        if (b) _canvasBlocks.push({ label: b.label, text: b.text, cat: b.cat, collapsed: false });
      });
      renderCanvas();
    });
