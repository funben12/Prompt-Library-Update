...
def create_role():
    ...
    try:
        cur = conn.execute('''
            INSERT INTO roles (name, icon, colour, persona, tone, expertise, example_phrase, knowledge_base)
            VALUES (?,?,?,?,?,?,?,?)
        ''', (p['name'], p['icon'], p['colour'], p['persona'],
              p['tone'], p['expertise'], p['example_phrase'], p['knowledge_base']))
        rid = cur.lastrowid
        conn.commit()
    finally:
        conn.close()
        ...
        conn.execute('''
            UPDATE roles
               SET name=?, icon=?, colour=?, persona=?, tone=?, expertise=?,
                   example_phrase=?, knowledge_base=?, updated_at=CURRENT_TIMESTAMP
             WHERE id=?
        ''', (p['name'], p['icon'], p['colour'], p['persona'],
              p['tone'], p['expertise'], p['example_phrase'], p['knowledge_base'], rid))
        conn.commit()
    finally:
        conn.close()
    return jsonify({'success': True})
...
def duplicate_role(rid):
    ...
    try:
        ...
        cur = conn.execute(
            'INSERT INTO roles (name, icon, colour, persona, tone, expertise, example_phrase, knowledge_base) VALUES (?,?,?,?,?,?,?,?)',
            (r['name'] + ' (Copy)', r['icon'], r['colour'], r['persona'],
             r['tone'], r['expertise'], r['example_phrase'], kb_json)
        )
        new_id = cur.lastrowid
        conn.commit()
    finally:
