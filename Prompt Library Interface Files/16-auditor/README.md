...
| File | What it is |
|---|---|
| `markup.html` | The exact HTML block for this view, dedented. Root element: `#auditWorkspace` |
| `styles.css` | CSS rules from `static/app.css` whose selectors target this view's ids / view-specific classes (14 rules) |
| `script.js` | Top-level functions from `static/app.js` that reference this view's ids or carry its name prefix (2 functions) |
| `preview.html` | Standalone page that loads `_shared` + the three files above, for isolated visual work |

## Source locations

- HTML: `static/index.html`, element `#auditWorkspace`
- CSS: `static/app.css`
